...
interface EditCaseFormProps {
  case_: CaseInterface;
  onSuccess?: (updatedCase: CaseInterface) => void;
  onCancel?: () => void;
}

export function EditCaseFormComponent({ case_, onSuccess, onCancel }: EditCaseFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      status: case_.status,
      filedAt: formatDateForInput(case_.filedAt),
    },
  });

  const onSubmit = async (data: EditCaseForm) => {
    setIsLoading(true);
    setError("");
    setSuccess(false);
    ...
    try {
      const cleanedData = {
        ...
        filedAt: new Date(data.filedAt).toISOString(),
      };

      const updatedCase = await caseService.update(case_._id, cleanedData);
      setSuccess(true);
